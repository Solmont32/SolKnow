...
### 2.1 四边形不等式 (Quadrangle Inequality)
对于 $a < b < c < d$，若 $f[a][c] + f[b][d] \le f[a][d] + f[b][c]$，则称 $f$ 满足四边形不等式。

### 2.2 决策单调性
若满足上述性质，可证明最优分割点 $s[i][j]$ 满足：
$$s[i][j-1] \le s[i][j] \le s[i+1][j]$$

<div className="bg-amber-50 border-l-4 border-amber-500 p-4 my-4">
  <p className="font-bold flex items-center"><Zap className="mr-2 text-amber-500" /> 复杂度飞跃</p>
  <p>利用该性质，在枚举分割点 $k$ 时，只需在 $[s[i][j-1], s[i+1][j]]$ 范围内查找。总复杂度经均摊分析为 $O(N^2)$。</p>
</div>
