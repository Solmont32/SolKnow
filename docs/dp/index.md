---
title: 动态规划 (Dynamic Programming) 系统建模
sidebar_position: 1
---

import { Code2, Zap, Target, Layers, GitBranch, Binary, Infinity, Trophy, Microscope, ShieldCheck } from 'lucide-react';

# 动态规划 (Dynamic Programming) 系统建模

> **"A problem well-stated is a problem half-solved."** —— 动态规划不仅是一种算法，更是一种**多阶段决策过程的数学建模范式**。
...
- **初始化 (Base Case)**：定义最简单子问题的解（如 $dp[0] = 0$）。
...
| <GitBranch size={18} className="inline-block mr-1 text-green-500" /> [树形 DP](tree-dp) | 子树规约、换根技术 | $O(N)$ | 层次拓扑依赖 |
