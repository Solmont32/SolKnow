...
**命题**：在树上背包问题中，节点 $u$ 在容量 $j$ 下的最优解由其子树的最优分配构成。

**证明**：
设节点 $u$ 有子节点 $v_1, v_2, \dots, v_k$。令 $g(u, i, j)$ 表示考虑前 $i$ 棵子树，分配总容量为 $j$ 时的最大价值。
$$g(u, i, j) = \max_{0 \le k \le j} \{ g(u, i-1, j-k) + f(v_i, k) \}$$
这本质上是一个**分组背包问题**，其中每一棵子树 $T_{v_i}$ 都是一个物品组。根据背包问题的最优子结构性质，该模型成立。证毕。

### 2.2 典型转移模式
- **独立集类**：$f[u][0] = \sum \max(f[v][0], f[v][1]), \quad f[u][1] = w_u + \sum f[v][0]$。
- **路径类**：$f[u]$ 维护以 $u$ 为终点的最长链，全局最优解在合并两条链时更新。
