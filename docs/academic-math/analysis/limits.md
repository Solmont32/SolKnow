...
本章围绕 $\epsilon$-$N$ 定义、收敛判别与经典计算模板展开，重点训练“定义证�?+ 判别工具 + 计算技巧”三条主线�?

## 一、定义与基本性质

### 1. $\epsilon$-$N$ 定义

若对任意 $\epsilon>0$，存�?$N\in\mathbb{N}_+$，使得当 $n>N$ 时恒�?
...
定义中的量词顺序必须严格：先“任�?$\epsilon$”，再“存�?$N$”。证明题中交换顺序通常会导致错误结论�?
</KnowledgeCard>
...
�?$x_1=1,\ x_{n+1}=\frac12\left(x_n+\frac{2}{x_n}\right)$，求极限�?
...
证明调和级数部分�?$H_n=\sum_{k=1}^n\frac1k$ 不收敛�?
...
故存在固定正�?$\epsilon_0=\frac12$，使任意大下标仍可找到两项差值超�?$\epsilon_0$，违背柯西准则�?
...
�?$\epsilon$-$N$ 定义证明 $\lim\limits_{n\to\infty}\frac{n+1}{n}=1$�?
...
�?$X_n=\sum_{k=1}^n a_k,\ Y_n=n$，用 Stolz�?
...
�?$u_1>0$�?u_{n+1}=\frac{u_n+3}{u_n+1}$。证明其收敛并求极限�?
...
考察映射 $\varphi(x)=\frac{x+3}{x+1}$ �?$(0,+\infty)$ 上，
