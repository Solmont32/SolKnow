...
$$ \text{Res}(f, z*0) = \frac{1}{2\pi i} \oint_C f(z) \, dz $$
在 Laurent 展开中，$\text{Res}(f, z_0) = c*{-1}$。

### 2. 计算公式（$m$ 阶极点）

若 $z_0$ 是 $f(z)$ 的 $m$ 阶极点，则：
$$ \text{Res}(f, z*0) = \frac{1}{(m-1)!} \lim*{z \to z_0} \frac{d^{m-1}}{dz^{m-1}} \left[ (z - z_0)^m f(z) \right] $$
...
$$ \oint*C f(z) \, dz = 2\pi i \sum*{k=1}^n \text{Res}(f, z_k) $$
