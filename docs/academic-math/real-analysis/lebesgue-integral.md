...
*注：这里 $m(E_i)$ 为 Lebesgue 测度。若某个 $a_i = 0$ 且 $m(E_i) = \infty$，约定 $0 \cdot \infty = 0$。*
...
2. **寻找控制函数 $G(x)$**：
   ...
   在 $(0, 1]$ 上，$\int_0^1 \frac{1}{2 \sqrt{x}} dx = [\sqrt{x}]_0^1 = 1 < \infty$，即 $G(x) = \frac{1}{2\sqrt{x}}$ 是可积的控制函数。
3. **应用 DCT**：
   由于存在控制函数，极限号可进入积分：
