...
#### 解析
1. **分区间讨论极限**：
   - 当 $0 \le x < 1$ 时，$x^n \to 0$，故 $f(x) = \frac{-1}{1} = -1$。
   - 当 $x = 1$ 时，$x^n = 1$，故 $f(x) = \frac{1-1}{1+1} = 0$。
   - 当 $x > 1$ 时，$x^n \to \infty$，故 $f(x) = \lim \frac{1 - 1/x^n}{1 + 1/x^n} = 1$。
2. **分析间断点**：
   在 $x=1$ 处：
   - 左极限 $f(1^-) = -1$。
   - 右极限 $f(1^+) = 1$。
   - 函数值 $f(1) = 0$。
3. **结论**：$x=1$ 是第一类跳跃间断点。函数在 $[0, 1)$ 和 $(1, +\infty)$ 上连续。
</details>

---

### 练习 34：利用零点定理证明根的存在性
证明：任意奇数次实系数多项式至少有一个实根。

<details>
<summary>点击查看解析</summary>

#### 解析
1. **设多项式为** $P(x) = a_n x^n + a_{n-1} x^{n-1} + \dots + a_0$，其中 $n$ 为奇数，$a_n \neq 0$。
2. **考察趋向性**：
   - 当 $x \to +\infty$ 时，$P(x) \to \text{sgn}(a_n) \cdot \infty$。
   - 当 $x \to -\infty$ 时，由于 $n$ 是奇数，$P(x) \to -\text{sgn}(a_n) \cdot \infty$。
3. **应用零点定理**：
   存在足够大的正数 $M$，使得 $P(M)$ 与 $P(-M)$ 符号相反。
   由于 $P(x)$ 在 $[-M, M]$ 上连续，由零点定理，必存在 $\xi \in (-M, M)$ 使得 $P(\xi) = 0$。
</details>

---

### 练习 35：介值定理的应用（海拔对跖点）
证明：地球大圆路径上必存在两个对跖点，其海拔高度相等。

<details>
<summary>点击查看解析</summary>

#### 解析
1. **模型化**：设圆周长为 $L$，位置用 $x \in [0, L]$ 表示，$x$ 与 $x + L/2$ 为对跖点。
2. **定义海拔函数**：设 $h(x)$ 为位置 $x$ 的海拔，假设 $h(x)$ 是连续的且 $h(0) = h(L)$。
3. **构造辅助函数**：令 $f(x) = h(x) - h(x + L/2)$，考虑区间 $[0, L/2]$。
4. **考察端点值**：
   - $f(0) = h(0) - h(L/2)$。
   - $f(L/2) = h(L/2) - h(L) = h(L/2) - h(0) = -f(0)$。
5. **应用零点定理**：
   - 若 $f(0) = 0$，则 $0$ 和 $L/2$ 即为所求。
   - 若 $f(0) \neq 0$，则 $f(0)$ 与 $f(L/2)$ 异号。存在 $\xi \in (0, L/2)$ 使得 $f(\xi) = 0$。
6. **结论**：即 $h(\xi) = h(\xi + L/2)$，对跖点海拔相等。
</details>

---

### 练习 36：双连续函数的性质
设 $f \in C[a, b]$，且 $f(x)$ 是一一映射。证明：$f(x)$ 在 $[a, b]$ 上必是严格单调的。
...
#### 解析
1. **反证法**：假设 $f$ 不是严格单调的。
2. **构造反例点**：则必存在 $x_1 < x_2 < x_3$，使得 $f(x_2)$ 不是介于 $f(x_1)$ 与 $f(x_3)$ 之间。
3. **情形讨论**：
   假设 $f(x_1) < f(x_3) < f(x_2)$。
4. **应用介值定理**：
   由于 $f$ 在 $[x_1, x_2]$ 上连续，且 $f(x_1) < f(x_3) < f(x_2)$，由介值定理，存在 $\xi \in (x_1, x_2)$ 使得 $f(\xi) = f(x_3)$。
5. **矛盾**：这与 $f$ 是一一映射（$\xi \neq x_3$ 但函数值相等）矛盾。
</details>
...
### 练习 37：一致连续性的判定
证明 $f(x) = x \sin x$ 在 $[0, +\infty)$ 上不一致连续。
...
2. **距离**：$|x_n - y_n| = 1/n \to 0$。
3. **值差**：
   $|f(x_n) - f(y_n)| = |(2n\pi + \frac{1}{n}) \sin(\frac{1}{n}) - 0| \approx 2n\pi \cdot \frac{1}{n} = 2\pi$。
4. **结论**：当 $n \to \infty$ 时，值差不趋于 0，故不一致连续。
</details>

---

### 练习 38：一致连续性的性质
证明：若 $f(x)$ 在 $[a, +\infty)$ 上连续，且存在水平渐近线 $\lim_{x \to +\infty} f(x) = A$，则 $f(x)$ 在 $[a, +\infty)$ 上一致连续。

<details>
<summary>点击查看解析</summary>

#### 解析
1. **利用极限定义**：对 $\forall \epsilon > 0$，存在 $M > a$，使得当 $x > M$ 时，$|f(x) - A| < \epsilon/3$。
2. **分段讨论**：
   - 在闭区间 $[a, M+1]$ 上，$f$ 连续，由 **Cantor 定理** 知其一致连续。存在 $\delta_1 > 0$。
   - 在 $(M, +\infty)$ 上，对任意 $x, y$，有 $|f(x) - f(y)| \le |f(x) - A| + |f(y) - A| < 2\epsilon/3 < \epsilon$。
3. **取 $\delta = \min\{\delta_1, 1\}$**：结合两段即可证得全区间一致连续。
</details>

---
本板块旨在通过深度证明题，训练对罗尔定理、拉格朗日中值定理及柯西中值定理的综合运用能力。

### 综合证明 1：方程根的唯一性证明（罗尔定理）
证明：方程 $e^x = ax + b$ ($a > e$) 在 $\mathbb{R}$ 上最多有两个实根。

<details>
<summary>点击查看解析</summary>

#### 证明过程
1. **构造函数**：设 $f(x) = e^x - ax - b$。
2. **反证法**：假设 $f(x)$ 有三个实根 $x_1 < x_2 < x_3$。
3. **应用罗尔定理**：
   - 在 $[x_1, x_2]$ 上，$f(x)$ 满足罗尔定理，存在 $\xi_1 \in (x_1, x_2)$ 使得 $f'(\xi_1) = 0$。
   - 在 $[x_2, x_3]$ 上，$f(x)$ 满足罗尔定理，存在 $\xi_2 \in (x_2, x_3)$ 使得 $f'(\xi_2) = 0$。
4. **考察导数**：
   $f'(x) = e^x - a$。
   令 $f'(x) = 0 \implies e^x = a \implies x = \ln a$。
   由于 $f'(x)$ 是单调递增函数（或通过二阶导 $f''(x) = e^x > 0$ 可知），$f'(x)$ 在 $\mathbb{R}$ 上最多只有一个零点。
5. **结论**：这与存在两个零点 $\xi_1, \xi_2$ 矛盾。故原方程最多有两个实根。
</details>

---

### 综合证明 2：导数中值点的存在性（辅助函数法）
设 $f(x)$ 在 $[0, 1]$ 上连续，在 $(0, 1)$ 内可导，且 $f(1) = 0$。证明：存在 $\xi \in (0, 1)$，使得 $f'(\xi) = -\frac{f(\xi)}{\xi}$。
...
#### 证明过程
1. **整理欲证式**：
   $f'(\xi) = -\frac{f(\xi)}{\xi} \implies \xi f'(\xi) + f(\xi) = 0$。
2. **观察结构**：
   注意到 $\xi f'(\xi) + f(\xi)$ 恰好是函数 $g(x) = x f(x)$ 的导数形式，即 $(xf(x))' = f(x) + xf'(x)$。
3. **构造辅助函数**：令 $g(x) = x f(x)$。
4. **验证罗尔定理条件**：
   - $g(x)$ 在 $[0, 1]$ 上连续（两个连续函数之积）；
   - $g(x)$ 在 $(0, 1)$ 内可导；
   - $g(0) = 0 \cdot f(0) = 0$；
   - $g(1) = 1 \cdot f(1) = 0$（已知条件）。
5. **应用定理**：由罗尔定理，存在 $\xi \in (0, 1)$ 使得 $g'(\xi) = 0$。
   即 $f(\xi) + \xi f'(\xi) = 0 \implies f'(\xi) = -\frac{f(\xi)}{\xi}$。
</details>

---

### 综合证明 3：拉格朗日中值定理与不等式
证明：对于任意 $0 < b < a$，有 $\frac{a-b}{a} < \ln\frac{a}{b} < \frac{a-b}{b}$。

<details>
<summary>点击查看解析</summary>

#### 证明过程
1. **构造函数**：设 $f(x) = \ln x$，考虑区间 $[b, a]$。
2. **应用拉格朗日中值定理**：
   存在 $\xi \in (b, a)$，使得：
   $$\frac{f(a) - f(b)}{a - b} = f'(\xi) = \frac{1}{\xi}$$
   即 $\frac{\ln a - \ln b}{a - b} = \frac{1}{\xi} \implies \ln\frac{a}{b} = \frac{a-b}{\xi}$。
3. **利用中值点的范围**：
   由于 $b < \xi < a$，有 $\frac{1}{a} < \frac{1}{\xi} < \frac{1}{b}$。
4. **代入化简**：
   将 $\frac{1}{\xi} = \frac{\ln(a/b)}{a-b}$ 代入不等式：
   $$\frac{1}{a} < \frac{\ln(a/b)}{a-b} < \frac{1}{b}$$
   两边同乘 $(a-b) > 0$，得证：
...
### 综合证明 4：柯西中值定理的应用
设 $f(x), g(x)$ 在 $[a, b]$ 上连续，在 $(a, b)$ 内二阶可导。证明：若 $f(a)=f(b)=g(a)=g(b)=0$，则存在 $\xi \in (a, b)$，使得 $\frac{f''(\xi)}{g''(\xi)} = \frac{f(\xi)}{g(\xi)}$ 这种形式通常较难。我们换一个经典题目：
证明：若 $f(0)=0$，则存在 $\xi, \eta$ 满足某种比例... 
**修正为更具代表性的题目**：
已知 $f(x)$ 在 $[a, b]$ 上连续且可导，证明存在 $\xi \in (a, b)$ 使得 $2\xi(f(b)-f(a)) = f'(\xi)(b^2-a^2)$。

<details>
<summary>点击查看解析</summary>

#### 证明过程
1. **识别定理**：该式具有柯西中值定理的标准形式：
...
5. **整理**：$2\xi(f(b)-f(a)) = f'(\xi)(b^2-a^2)$。
</details>

---

### 综合证明 5：高阶导数与多点罗尔定理
设 $f(x)$ 在 $[a, b]$ 上有 $n$ 阶导数，且在 $x_1 < x_2 < \dots < x_{n+1}$ 共 $n+1$ 个点处取值为 0。证明：在 $(x_1, x_{n+1})$ 内至少存在一点 $\xi$，使得 $f^{(n)}(\xi) = 0$。
...
#### 证明过程
1. **第一步（对 $f(x)$ 应用罗尔定理）**：
   在 $n$ 个区间 $[x_i, x_{i+1}]$ ($i=1, \dots, n$) 上，每个区间至少存在一个 $\xi_{1, i}$ 使得 $f'(\xi_{1, i}) = 0$。
   这样我们得到了 $n$ 个导数的零点。
2. **第二步（对 $f'(x)$ 应用罗尔定理）**：
   在 $n-1$ 个由上述零点组成的区间上，每个区间至少存在一个 $\xi_{2, i}$ 使得 $f''(\xi_{2, i}) = 0$。
   我们得到了 $n-1$ 个二阶导数的零点。
3. **以此类推**：
   - 第 $k$ 步将得到 $n-k+1$ 个 $f^{(k)}(x)$ 的零点。
4. **最后一步**：
   当 $k = n$ 时，我们得到 $n-n+1 = 1$ 个 $f^{(n)}(x)$ 的零点。
   即存在 $\xi$ 使得 $f^{(n)}(\xi) = 0$。
</details>
