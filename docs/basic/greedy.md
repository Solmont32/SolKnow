...
### 2. 常用证明方法
由于贪心逻辑往往“直觉化”，严谨的证明至关重要：
- **微扰法 (Exchange Argument)**：假设存在一个最优解与贪心解不同，通过交换其中两个相邻元素，证明解不会变差。
- **范围归纳法**：证明贪心选择的第一步必然包含在某个最优解中。
...
### 1. 区间问题
- **区间选点**：给定多个区间，选择最少的点覆盖所有区间。  
  *策略：按右端点排序，每次选当前未覆盖区间的右端点。*
