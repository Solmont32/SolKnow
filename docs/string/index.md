...
1. **基础匹配与前缀函数**：从暴力匹配到 KMP，深入掌握前缀函数的递推与势能分析证明。
2. **回文对称性分析**：学习 Manacher 算法，利用半径对称性在线性时间内提取所有回文子串。
3. **哈希与随机化**：掌握多项式哈希，理解生日悖论与双哈希防御策略。
4. **多模式匹配自动机**：理解 AC 自动机的 Fail 指针与 Trie 图 DFA 化，掌握拓扑优化统计技巧。
5. **后缀结构巅峰**：深入理解 SAM 的 Endpos 等价类、Parent Tree 结构以及 SA 的 LCP Lemma。
