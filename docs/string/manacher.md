...
### 2.2 状态转移方程
对于当前计算的位置 $i$，若 $i \le R$，我们可以利用 $i$ 关于 $M$ 的对称点 $j = 2M - i$：
1. **Case 1**: $d[j] < R - i + 1$。此时以 $i$ 为中心的回文串完全包含在 $R$ 内部，故 $d[i] = d[j]$。
2. **Case 2**: $d[j] \ge R - i + 1$。此时以 $i$ 为中心的回文串至少延伸到 $R$，故 $d[i] \ge R - i + 1$，需继续向外暴力扩展。

综合转移式：
$$ d[i] = \min(d[2M - i], R - i + 1) $$
...
3. 利用 $d[i]$ 更新 $L$ 和 $R$ 的边界值，然后通过线性递推补全。
4. 最终答案为 $\max_{i=1}^{n-1} (L[i] + R[i+1])$。
...
// 3. 递推：以 i 结尾的最长回文，去掉两端字符也是回文且以 i-1 结尾
for (int i = n_new - 1; i >= 1; i -= 2) L[i-2] = max(L[i-2], L[i]-2);
for (int i = 1; i <= n_new - 1; i += 2) R[i+2] = max(R[i+2], R[i]-2);
// 4. 计算结果
```
</details>
...
**思路**：
该问题等价于找到 $S$ 的最长回文后缀。在 Manacher 预处理串中找到一个包含末尾字符且半径最大的回文中心。
...
int solve(string s) {
    ...
    for (int i = n_new - 1; i >= 0; i--) {
        if (i + d[i] == n_new) { // 该回文串触及预处理串的末尾
            return n - (d[i] - 1); // 原串长度减去回文后缀长度
        }
    }
}
```
