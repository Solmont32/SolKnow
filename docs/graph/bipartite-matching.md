...
### 1. Kőnig 定理：匹配与覆盖的对偶
在二分图中：
$$\text{最大匹配数 (Max Matching)} = \text{最小顶点覆盖数 (Min Vertex Cover)}$$
**证明概要 (Duality Proof)**：
1. 显然，最大匹配 $M$ 的边需要至少 $|M|$ 个点来覆盖。
2. 构造性证明：从左部非匹配点出发走增广路，标记访问过的点。取左部**未访问**点与右部**已访问**点即得等大的覆盖集。
3. 这一结论揭示了二分图在 LP (Linear Programming) 意义下的全单模性，即离散优化与连续优化的等效性。

### 2. Hall 结婚定理 (Hall's Marriage Theorem)
二分图 $G=(L \cup R, E)$ 存在覆盖 $L$ 的匹配的充要条件是：
$$\forall S \subseteq L, |N(S)| \ge |S|$$
其中 $N(S)$ 是 $S$ 的邻域。
- **必要性**：显然，每个 $S$ 中的点至少需要对应一个邻居。
- **充分性**：若不满足最大匹配等于 $|L|$，由最大流最小割定理可导出一个违反该条件的子集 $S$。
