...
<KnowledgeCard title="SCC 单调性与拓扑序验证 (Monotonicity Verification)" icon={<ShieldCheck size={20} />}>
**1. 栈的单调性**：
在 Tarjan 过程中，栈内节点的 $dfn$ 值是严格递增的。当发现 $low[u] = dfn[u]$ 时，栈顶到 $u$ 的所有点构成一个 SCC。
**2. 拓扑序单调性 (SCC ID Properties)**：
Tarjan 算法求出的 SCC 编号顺序，恰好是原图缩点后构成的 DAG 的**反向拓扑序**。
*验证*：
- 考虑两个 SCC $C_1, C_2$，存在边 $C_1 \to C_2$。
- 在 DFS 过程中，由于 $C_2$ 在拓扑序后方，$C_2$ 会在 $C_1$ 的递归回溯过程中更早地被完整识别（即 $low = dfn$ 满足），从而更早出栈并分配更小的 `scc_id`。
- 因此，`scc_id[C_2] < scc_id[C_1]`，这在 2-SAT 构造解时至关重要。
</KnowledgeCard>
